...
/*
  The masthead now runs the full width of the window and carries the name, so the
  sidebar starts at the navigation. It sits below the masthead rather than beside
  it, which is why it is sticky-and-self-start rather than fixed: a stretched flex
  item cannot stick, and a fixed one would need the masthead's height hard-coded in
  half a dozen places.
*/
export default function Sidebar({ meta }: Props) {
  return (
    <aside className="sticky top-0 hidden h-screen w-[268px] shrink-0 self-start overflow-y-auto border-r border-line bg-surface scroll-thin lg:block">
      <SidebarBody meta={meta} />
    </aside>
  );
}

/** The same navigation on a narrow screen: a bar, and a slide-over behind it. */
export function MobileNav({ meta }: Props) {
    ...
    <>
      <div className="sticky top-0 z-40 flex items-center gap-3 border-b border-line bg-surface/90 px-4 py-2.5 backdrop-blur lg:hidden">
        ...
        <span className="text-[0.8125rem] font-medium text-fg-muted">
          {NAV.find((n) => n.href === pathname)?.label ?? "Menu"}
        </span>
...
function SidebarBody({ meta, onNavigate }: Props & { onNavigate?: () => void }) {
  const pathname = usePathname();
